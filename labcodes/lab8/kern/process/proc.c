...
alloc_proc(void) {
     ...
     */
    //LAB8:EXERCISE2 2015011371 HINT:need add some code to init fs in proc_struct, ...
		proc->filesp = NULL;
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
	...
	ret = proc->pid;			//    7. set ret vaule using child proc's pid
	 //LAB8:EXERCISE2 2015011371  HINT:how to copy the fs in parent's proc_struct?
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2015011371  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program
        while (start < end) {
			...
			load_icode_read(fd,page2kva(page) + off,size,now_offset);
            start += size, now_offset += size;
        }
